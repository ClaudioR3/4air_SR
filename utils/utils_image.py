...
def npyssave(ndarrs, npy_path, filename_list):
    """
    npys: list, N numpy of size WxHxC
    """
    scale=5
    path = '{}/x{}'.format(npy_path, scale)
    os.makedirs(path, exist_ok=True)
    
    # only for 3D
    normalized = ndarrs.data.mul(255 / 255)
    normalized = normalized.permute(1,2,0).cpu().numpy()
    filepath = os.path.join(path, filename_list[0])
    np.save(filepath, normalized)
    
    """
    for i, (ndarr, filename) in enumerate(zip(ndarrs, filename_list)):
        normalized = ndarr.data.mul(255 / 255)
    ...
    #    np.save('{}.npy'.format(filename), ndarr)
...
# MSE & PSNR from RCAN
# --------------------------------------------
def calc_mse_psnr(sr, hr, scale, rgb_range):
    diff = (sr - hr).data.div(rgb_range)
    shave = scale
    """
    if diff.size(1) > 1:
...
# MSE
# --------------------------------------------
def calculate_mse(sr, hr, rgb_range, border=0):
    '''calculate MAE
    ...
    if not sr.shape == hr.shape:
        raise ValueError('Input images must have the same dimensions.')
    #h, w = sr.shape[:2]
    #sr = sr[border:h - border, border:w - border]
    #hr = hr[border:h - border, border:w - border]
    diff = (sr - hr).data.div(rgb_range)
    valid = diff[:, shave:-shave, shave:-shave]
    mse = valid.pow(2).mean()
    return mse
